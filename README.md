...
Files are saved to a data/ directory that is checked for and run on the first search in OASIS. You can override this using relative paths (`../../save_here/file.csv`) as the file save prepends data/ as well as appends the source and search strategy to the file name. Additionally a log file is created in logs/ tha captures when and what query qas searched - this can be used to replicate searches down the line.
