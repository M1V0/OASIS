...
ArXiv, PsyArXiv, SocArXiv, engrXiv, LawArXiv, MedArXiv, ECSarXiv, and Thesis Commons
...
Files are saved to a datafile/ directory that is checked for and run on the first search in OASIS. You can override this using relative paths (`../../save_here/file.csv`) as the file save prepends data/ as well as appends the source and search strategy to the file name. Additionally a log file is created in logs/ tha captures when and what query has searched - this can be used to replicate searches down the line.

Note: there is currently no routine in place that considers rate limits - so play nice.
